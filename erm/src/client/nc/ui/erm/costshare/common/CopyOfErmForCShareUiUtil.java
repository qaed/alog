 ...
 * @author chenshuaia
 */
public class CopyOfErmForCShareUiUtil {
	/**
		...
		if (ArrayUtils.indexOf(AggCostShareVO.getBodyMultiSelectedItems(), eve.getKey(), 0) >= 0) {
			...
			if (!isChanged) {
				for (int i = 0; i < cardPanel.getRowCount(); i++) {
					CopyOfErmForCShareUiUtil.setRateAndAmount(i, cardPanel);
				}
			} else {

...
public class ERMBillCheckValidator implements Validator {
	...
	private void checkBillContrast(JKBXVO bxvo) throws BusinessException {
		if (BXConstans.BXRB_CODE.equals(getModel().getContext().getNodeCode())) {
			...
			if (contrastVO == null || (contrastVO != null && contrastVO.length == 0)) {
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011", "UPP2011-000919")/**
				 * @res
				 *      * "还款单没有冲借款不可以保存！"
				 */
				);
		...
		if (!bxvo.getParentVO().djlxbm.equals(BXConstans.BILLTYPECODE_RETURNBILL) && bxvo.getParentVO().getDjdl().equals(BXConstans.BX_DJDL)) {
			BxcontrastVO[] contrastVO = bxvo.getContrastVO();
				...
				if (childrenVO != null && childrenVO.length != 0) {
					for (BXBusItemVO bxBusItemVO : childrenVO) {
						if (bxBusItemVO.getYbje().compareTo(new UFDouble(0)) <= 0) {
							throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011", "UPP2011-000905")/*
																																		 * @
																																		 * res
		...
		if (contrastVO != null && contrastVO.length != 0) {
			for (BxcontrastVO bxcontrastVO : contrastVO) {
				if (bxcontrastVO.getCjkybje().compareTo(new UFDouble(0)) <= 0) {
					throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011", "UPP2011-000918")/*
																																 * @
																																 * res
	...
	private void checkBillDate(JKBXVO bxvo) throws BusinessException {
		...
		if (bxvo instanceof JKVO) {// 最迟还款日期不能早于单据日期
			...
			if (zhrq != null) {
				if (bxdjrq.afterDate(zhrq)) {
					throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2006030102", "UPP2006030102-001122"))/*
																																			 * 单据日期不可以晚于最迟还款日期
																																			 * *
																																			 */;
				}
		...
		if (pkItem != null) {
			AggMatterAppVO aggMatterVO = NCLocator.getInstance().lookup(IErmMatterAppBillQuery.class).queryBillByPK(pkItem);
			...
			if (approvetime != null && approvetime.afterDate(bxdjrq)) {
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2006030102", "UPP2006030102-001121"))/*
																																		 * 单据日期不可以早于费用申请单的日期
																																		 * *
																																		 */;
			}
		...
		if (bxvo.getParentVO().getDjdl().equals(BXConstans.BX_DJDL)) {
			...
			for (int i = 0; i < cShareVos.length; i++) {
				...
				for (int j = 0; j < attributeNames.length; j++) {
					if (getNotRepeatFields().contains(attributeNames[j]) || attributeNames[j].startsWith(BXConstans.BODY_USERDEF_PREFIX)) {
				...
				if (!controlKeys.contains(controlKey.toString())) {
					...
					throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("201107_0", "0201107-0114")/*
																																	 * @
																																	 * res
																																	 * "分摊明细信息存在重复行！"
																																	 */);
		...
		if (bxvo.getParentVO().getIsexpamt().equals(UFBoolean.TRUE)) {
			if (nc.vo.er.util.StringUtils.isNullWithTrim(bxvo.getParentVO().getStart_period())) {
				...
				if (startperiod_enddate.compareTo(djrq) < 0) {
					throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("201107_0", "0201107-0110")/*
																																	 * @
																																	 * res
																																	 * "开始摊销期间应大于单据日期"
																																	 */);
			...
			for (BXBusItemVO child : childrenVO) {
				...
				if (bxvo.getParentVO().getDjdl().equals(BXConstans.JK_DJDL)) {
					if (child.getYbje().compareTo(UFDouble.ZERO_DBL) <= 0 && child.getCjkybje().compareTo(UFDouble.ZERO_DBL) <= 0) {
						throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011v61013_0", "02011v61013-0088")/*
																																				 * @
																																				 * res
				...
				if (bxvo.getParentVO().getDjdl().equals(BXConstans.BX_DJDL)) {
					if (child.getYbje().compareTo(UFDouble.ZERO_DBL) == 0 && child.getCjkybje().compareTo(UFDouble.ZERO_DBL) == 0) {
						throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011v61013_0", "02011v61013-0090")/*
																																				 * @
																																				 * res
	...
	private void checkHeadFinItemJe(JKBXVO bxvo) throws ValidationException {
		...
		String[] name =
				new String[] { nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011", "UPP2011-000280")/*
																											 * @
																											 * res
																											 * "原币金额"
		...
		for (int j = 0; j < length; j++) {
			...
			UFDouble headJe =
					parentVO.getAttributeValue(keys[j]) == null ? new UFDouble(0) : (UFDouble) parentVO.getAttributeValue(keys[j]);
			...
			if (headJe.compareTo(bodyJe) != 0) {
				...
				throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011", "UPP2011-000325", null, new String[] { name[j] })/*
																																							 * @
																																							 * res
																																							 * "表头"
																																							 * i
	...
	private void checkHeadItemJe(JKBXVO bxvo) throws ValidationException {
		...
		if (!bxvo.getParentVO().getDjlxbm().equals(BXConstans.BILLTYPECODE_RETURNBILL)) {
			if (total != null && total.compareTo(UFDouble.ZERO_DBL) == 0) {
				...
				throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011", "UPP2011-000916")/*
																															 * @
																															 * res
																															 * "表头“合计金额”不能为0!"
																															 */);
			...
			if (ybje != null && ybje.compareTo(UFDouble.ZERO_DBL) == 0) {
				throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("2011", "UPP2011-000917")/*
																															 * @
																															 * res
																															 * "表头“财务核报金额”不能为0!"
																															 */);
	...
	private void checkFinRange(JKBXVO bxvo, JKBXHeaderVO parentVO) throws ValidationException {
		if (bxvo.getParentVO().getDjdl().equals(BXConstans.BX_DJDL)) {
			...
			if (range.doubleValue() < 0) { // 只能改小不能改大
				if (ybje > total) {
					throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0", "02011002-0145")/*
																																		 * @
																																		 * res
																																		 * "单据总金额不能改大!"
																																		 */);
		...
		if (bxvo.getParentVO().getDjdl().equals(BXConstans.BX_DJDL)) {
			...
			for (int i = 0; i < cShareVos.length; i++) {
				...
				if (!ErmForCShareUtil.isUFDoubleGreaterThanZero(shareAmount)) {
					throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("201107_0", "0201107-0112")/*
																																	 * @
																																	 * res
																																	 * "分摊信息不能包括金额小于等于0的行！"
																																	 */);
				...
				if (!ErmForCShareUtil.isUFDoubleGreaterThanZero(shareRatio)) {
					throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("201107_0", "0201107-0113")/*
																																	 * @
																																	 * res
																																	 * "分摊信息不能包括比例小于等于0的行！"
																																	 */);
		...
		if (!BXConstans.BXRB_CODE.equals(getModel().getContext().getNodeCode())) {
			...
			if (BXConstans.BX_DJDL.equals(currentDjLXVO.getDjdl())) {
				if (parentVO.getTotal().compareTo(UFDouble.ZERO_DBL) == 0) {
					// begin-ncm-liuxyk-NC2015031900408 取消还款单判断表体金额不能为0的校验
					if (!BXConstans.BILLTYPECODE_RETURNBILL.equals(currentDjLXVO.getDjlxbm())) {
						throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0", "02011002-0175")/*
																																			 * @
																																			 * res
																																			 * "报销单表头金额不可以等于0！"
																																			 */);
					}
					// end-ncm-liuxyk-NC2015031900408
				}
			} else {
				// 借款单金额都要大于0
				if (parentVO.getTotal().compareTo(UFDouble.ZERO_DBL) <= 0) {
					throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0", "02011002-0174")/*
																																		 * @
																																		 * res
																																		 * "借款单表头金额要大于0！"
																																		 */);
	...
	private void checkValidChildrenVO(BXBusItemVO[] childrenVO) throws ValidationException {
		...
		for (BXBusItemVO child : childrenVO) {
			...
			if (child.getTablecode() == null) {
				throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0", "02011002-0144")/*
																																	 * @
																																	 * res
																																	 * "表体页签信息不能为空!"
																																	 */);
			...
			for (BXBusItemVO child : childrenVO) {
				...
				if (BXConstans.BX_DJDL.equals(currentDjLXVO.getDjdl())) {
					if (child.getBbje().compareTo(UFDouble.ZERO_DBL) == 0) {
						// begin-ncm-liuxyk-NC2015031900408 取消还款单判断表体金额不能为0的校验
						if (!BXConstans.BILLTYPECODE_RETURNBILL.equals(currentDjLXVO.getDjlxbm())) {
							throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0", "02011002-0173")/*
																																				 * @
																																				 * res
																																				 * "报销单表体页签的本币金额不可以等于0!"
																																				 */);
					...
					// 借款单金额都要大于0
					if (child.getBbje().compareTo(UFDouble.ZERO_DBL) <= 0) {
						throw new ValidationException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("expensepub_0", "02011002-0172")/*
																																			 * @
																																			 * res
																																			 * "借款单表体页签的本币金额都要大于0!"
																																			 */);

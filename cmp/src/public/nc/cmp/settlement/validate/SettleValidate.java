...
public class SettleValidate extends DefaultValidationService {
	...
	public static void validateNetPayNegativeRow(SettlementAggVO... aggVOs) throws BusinessException {
		...
		for (SettlementAggVO aggvo : aggVOs) {
			if (aggvo.getChildrenVO() == null) {
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0584")/*
																																 * @
																																 * res
																																 * "结算信息数据异常，请检查该数据！"
																																 */);
		...
		UFDouble count = UFDouble.ZERO_DBL;// 20170713tsy
		for (SettlementBodyVO body : bodys) {

			// 负数判断
			// 20170713 tsy 去除表体负数校验，增加总额负数校验
			// if (body.getDirection() == CmpConst.Direction_Pay &&
			// UFDouble.ZERO_DBL.compareTo(body.getPaylocal()) > 0) {
			// throw new
			// BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0",
			// "03607set-0585"));
			/*
			 * @ res "支付金额为负数的单据不能网上支付！"
			 */
			//
			// }
			count = count.add(body.getPaylocal());// 合计金额
			// 20170713 end
			if (Integer.valueOf(4).equals(body.getTradertype()) && (body.getOppaccname() == null || body.getOppaccount() == null)) {
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607settle4_0", "03607settle4-0010")/*
																																		 * @
																																		 * res
																																		 * "结算方式为网银属性并且对方为散户时，对方银行账户和对方账户户名必填!"
																																		 */);
			} else if (!"DS".equals(body.getPk_billtype())) {
				if (body.getPk_oppaccount() == null && (body.getOppaccname() == null || body.getOppaccount() == null)) {
					throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0857")/*
																																	 * @
																																	 * res
																																	 * "结算方式为网银属性时，收款银行账户和收款账户户名必填"
																																	 */);
				}
		...
		// 20170713 tsy 总额校验
		if (UFDouble.ZERO_DBL.compareTo(count) > 0) {
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0585"));/*
																															 * @
																															 * res
																															 * "支付金额为负数的单据不能网上支付！"
																															 */
	...
	public static void validateNetPayState(SettlementAggVO... aggVOs) throws BusinessException {
		...
		for (SettlementAggVO aggvo : aggVOs) {
			...
			if (pk_user.equals(head.getCreator()) && pk_user.equals(head.getPk_signer())) {
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0858")/*
																																 * @
																																 * res
																																 * "不能一人同时进行支付、签字、创建操作"
																																 */);
			...
			if (head.getSettlestatus().equals(SettleStatus.SUCCESSSETTLE.getStatus())) {
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0587")/*
																																 * @
																																 * res
																																 * "已成功结算的单据不能进行网上转账"
																																 */);
			...
			if (head.getSettlestatus().equals(SettleStatus.SETTLERESET.getStatus())) {
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0588")/*
																																 * @
																																 * res
																																 * "结算红冲的单据不能进行网上转账"
																																 */);
			...
			// throw new
			// BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0","03607set-0589")/*@res
			// "单据已经支付过或则在支付中，不能进行网上转账"*/);
			// }
			// // 单据状态应为已生效，才能进行网上转账
			// if (head.getBusistatus() != BusiStatus.Effet.getBillStatusKind())
			// {
			// CheckException.checkArgument(head.getIsbusieffect().equals(UFBoolean.FALSE),
			// nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0","03607set-0590")/*@res
			// "只有生效的单据才能网上支付"*/);
			// }
		}
	}
		...
		if (isPay) {
			if (map.size() != 0) {
				for (String settleno : map.keySet()) {
					...
					SettlementHeadVO[] headVOs =
							InterfaceLocator.getInterfaceLocator().getSettlementQueryService().findHeadsBySettleNo(((SettlementHeadVO) aggs[0].getParentVO()).getPk_group(), ((SettlementHeadVO) aggs[0].getParentVO()).getPk_org(), settleno);

					if (!CheckException.checkArraysIsNull(headVOs)) {
						CheckException.checkArgument(headVOs.length != 1, nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0591")/*
																																							 * @
																																							 * res
																																							 * "同一批次结算号的单据必须一起合并支付"
																																							 */);
			...
			if (map.size() != 0) {
				if (map.size() == 1) {
					...
					SettlementHeadVO[] headVOs =
							InterfaceLocator.getInterfaceLocator().getSettlementQueryService().findHeadsBySettleNo(list.get(0).getPk_group(), list.get(0).getPk_org(), settleNo);
					// if(headVOs.length == list.size()) {
					//
					// }

					CheckException.checkArgument(headVOs.length != list.size(), nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0591")/*
																																									 * @
																																									 * res
																																									 * "同一批次结算号的单据必须一起合并支付"
																																									 */);

					CheckException.checkArgument(headVOs.length != aggs.length, nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0591")/*
																																									 * @
																																									 * res
																																									 * "同一批次结算号的单据必须一起合并支付"
																																									 */);
			...
			CheckException.checkArgument(map.size() > 1, nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0592")/*
																																			 * @
																																			 * res
																																			 * "结算号必须全部为空或则全部相等才能支付"
																																			 */);
	...
	public static void validatePayData(SettlementAggVO... aggVOs) throws BusinessException {
		for (SettlementAggVO aggVO : aggVOs) {
			for (SettlementBodyVO body : (SettlementBodyVO[]) aggVO.getChildrenVO()) {
				CheckException.checkArgument(body.getPk_balatype() == null, nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0593")/*
																																								 * @
																																								 * res
																																								 * "网上支付的单据结算方式不能为空"
																																								 */);
				...
				if (StringUtils.isEmpty(body.getPk_account())) {
					...
					throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607bacd_0", "03607bacd-0064")/*
																																	 * @
																																	 * res
																																	 * "本方账户不能为空!"
																																	 */);
		...
		for (SettlementAggVO settlementAggVO : aggVOs) {
			...
			for (SettlementBodyVO bodyVo : (SettlementBodyVO[]) settlementAggVO.getChildrenVO()) {
				if (!CmpConst.CONST_PK_CURRTYPE.equals(bodyVo.getPk_currtype())) {
					throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0768")/*
																																	 * @
																																	 * res
																																	 * "非人民币币种的记录不能进行支付！"
																																	 */);
	...
	public static void validateOBM(SettlementAggVO... aggVOs) throws BusinessException {
		for (SettlementAggVO aggVO : aggVOs) {
			SettlementHeadVO head = (SettlementHeadVO) aggVO.getParentVO();
			CheckException.checkArgument(CmpQueryModulesUtil.getOBMPeriod(head.getPk_org()) == null, nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0594")/*
																																														 * @
																																														 * res
	...
	public static void validateSupplier(SettlementAggVO... aggvos) throws BusinessException {
		for (SettlementAggVO aggvo : aggvos) {
			...
			if (bodys != null && bodys.length > 0) {
				...
				if (list_pk_trader.size() > 0) {
					SupplierPayFreezeUtil.checkSupplierPayFreeze(pk_org, list_pk_trader.toArray(new String[] {}));
				}
			}
		}
	}
				...
				for (SettlementBodyVO body : bodys) {
					...
					if (Integer.valueOf(4).equals(body.getTradertype())) {
						...
						if (StringUtils.isNullWithTrim(body.getOppaccount())) {
							sbBuilder.append(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set1_0", "03607set1-0121")/*
																																 * @
																																 * res
																																 * "对方银行账号不能为空，"
																																 */);
						...
						if (StringUtils.isNullWithTrim(body.getOppaccname())) {
							sbBuilder.append(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set1_0", "03607set1-0122")/*
																																 * @
																																 * res
																																 * "对方账户户名不能为空，"
																																 */);
						...
						if (StringUtils.isNullWithTrim(body.getOppbank())) {
							sbBuilder.append(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set1_0", "03607set1-0123")/*
																																 * @
																																 * res
																																 * "对方银行名称不能为空，"
																																 */);
						...
						if (body.getAccounttype() == null) {
							sbBuilder.append(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set1_0", "03607set1-0124")/*
																																 * @
																																 * res
																																 * "对方账户性质不能为空，"
																																 */);
						...
						if (sbBuilder.length() > 9) {
							...
							String err = nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set1_0", "03607set1-0120")/*
																															 * @
																															 * res
																															 * "对方类型为散户时："
																															 */
	...
	public static void validateNetPayFields(SettlementAggVO... aggvos) throws BusinessException {
		...
		for (SettlementAggVO aggvo : aggvos) {
			...
			if (Integer.valueOf(1).equals(settlementHeadVO.getDirection()) && !"DS".equals(settlementHeadVO.getPk_tradetype()) && bodys != null && bodys.length > 0) {
	...
	private static boolean isBalaTypeOfNetBank(String pk_balatype) {
		try {
			...
			ExceptionUtils.wrappBusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set1_0", "03607set1-0097")/*
																																		 * @
																																		 * res
																																		 * "获取结算方式失败"
																																		 */);
		...
		for (SettlementAggVO aggvo : aggvos) {
			for (SettlementBodyVO body : (SettlementBodyVO[]) aggvo.getChildrenVO()) {
				...
				if (StringUtil.isNotNull(body.getPk_notetype())) {
					...
					typeVO = NCLocator.getInstance().lookup(INotetypePubService.class).queryNotetypeVOByPk(body.getPk_notetype());
				}
				// 现金账户 与 银行账户, 不可以同时录入

				if (!StringUtils.isNullWithTrim(body.getPk_cashaccount()) && !StringUtils.isNullWithTrim(body.getPk_account())) {
					throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0822")/*
																																	 * @
																																	 * res
																																	 * "现金账户 与 银行账户, 不可以同时录入"
																																	 */);
				...
				if (!StringUtils.isNullWithTrim(body.getPk_cashaccount()) && !StringUtils.isNullWithTrim(body.getNotenumber())) {
					throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0823")/*
																																	 * @
																																	 * res
																																	 * "现金账户 与 票据信息, 不可以同时录入"
																																	 */);
				}
				if (StringUtil.isNotNull(body.getPk_account()) && typeVO != null && INotetypeConst.NOTETYPE_CLASS_BUSIPO == typeVO.getNoteclass()) {
					throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0911")/*
																																	 * @
																																	 * res
																																	 * "银行账户与商业汇票 不可以同时录入"
																																	 */);
				...
				if (!StringUtils.isNullWithTrim(body.getPk_notetype()) && StringUtils.isNullWithTrim(body.getPK_notenumber()) && StringUtils.isNullWithTrim(body.getNotenumber())) {
					throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607pub_0", "03607pub-0203")/*
																																	 * @
																																	 * res
																																	 * "表体行票据号不能为空"
																																	 */);
	...
	public static void validatePS(SettlementAggVO... aggVOs) throws BusinessException {
		for (SettlementAggVO settlementAggVO : aggVOs) {
			if (SettleUtils.isPsBillFrozen(settlementAggVO)) {
				throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0595")/*
																																 * @
																																 * res
																																 * "上游付款排程单据已被冻结，不能操作"
																																 */);
	...
	public static void validateAccountAndCurrencyIdentical(SettlementAggVO... aggvos) throws BusinessException {
		...
		IBankAccSubInfoQueryService bankAccSubInfoQueryService = NCLocator.getInstance().lookup(IBankAccSubInfoQueryService.class);
			...
			if (bodys != null && bodys.length > 0) {
				for (SettlementBodyVO body : bodys) {
					if (body.getPk_account() != null && body.getPk_cashaccount() != null) {
						throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0812"/* "同一表体行不允许同时存在本方账户和现金账户" */));
					}
					...
					if (body.getPk_currtype_last() == null || body.getPk_currtype().equals(body.getPk_currtype_last())) {
						...
						if (body.getPk_oppaccount() != null) {
							listAcc.add(body.getPk_oppaccount());
						}

						String pk_currtype = body.getPk_currtype();
						if (StringUtils.isNullWithTrim(body.getPk_account()) && StringUtils.isNullWithTrim(body.getPk_oppaccount())) {
							continue;
						}

						BankAccSubVO[] bankAccSubVOs = null;

						bankAccSubVOs = bankAccSubInfoQueryService.querySubInfosByPKs(CmpUtils.covertListToArrays(listAcc, String.class));

						if (bankAccSubVOs != null && bankAccSubVOs.length > 0) {
							if (!(pk_currtype != null && pk_currtype.equals(bankAccSubVOs[0].getPk_currtype()))) {
								throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0596")/*
																																				 * @
																																				 * res
																																				 * "银行账户的币种与币种字段不一致!"
																																				 */);
						...
						String pk_currtype = body.getPk_currtype();// 取最终付款信息
						...
						if (listAcc.size() > 0) {
							// 取对方账户信息
							bankAccSubVOs =
									bankAccSubInfoQueryService.querySubInfosByPKs(CmpUtils.covertListToArrays(listAcc, String.class));
						...
						if (listAcc_last.size() > 0) {
							bankAccSubVOs =
									bankAccSubInfoQueryService.querySubInfosByPKs(CmpUtils.covertListToArrays(listAcc_last, String.class));

							if (bankAccSubVOs != null && bankAccSubVOs.length > 0) {
								if (!(pk_currtype_last != null && pk_currtype_last.equals(bankAccSubVOs[0].getPk_currtype()))) {
									throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0596")/*
																																					 * @
																																					 * res
																																					 * "银行账户的币种与币种字段不一致!"
	 ...
	 * @throws BusinessException
	 */
	public static void validateBalance(boolean isOpp, boolean isCurrent, SettlementAggVO... aggVOs) throws BusinessException {
		...
		if (CmpUtils.isListNotNull(payBodyList)) {
			iSettlementService.checkBalance(CmpUtils.covertListToArrays(payBodyList, SettlementBodyVO.class), isOpp, isCurrent, null);
		}
	...
	public static void validateOppAcount(SettlementBodyVO[] bodys) throws BusinessException {
		List<String> custsuppBankAccMap = CmpUtils.makeList();
		List<String> psnBankAccMap = CmpUtils.makeList();
		...
		for (SettlementBodyVO body : bodys) {
			...
			if (CmpUtils.isObjectNull(objtype) || StringUtils.isNullWithTrim(pk_trader) || StringUtils.isNullWithTrim(bankAccount)) {
				// 没有对方信息或收款账户信息
				continue;
			}
			...
			if (objtype.equals(CmpConst.TradeObjType_CUSTOMER)) {
				...
				if (mapcustsup.get(bodys[i].getPk_trader() + " " + bodys[i].getPk_oppaccount()) == null) {
					sbmsg.append(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0816")/*
																													 ...
																													 * res
																													 * "第"
																													 */).append(i + 1).append(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("3607set_0", "03607set-0817")/*
																																																								 * @
																																																								 * res
																																																								 * "行对方单位与对方银行账户组织不一致\n"
																																																								 */);
	...
	private static Map<String, String> queryBankaccPksByCustPk(String... pk_custs) throws BusinessException {
		...
		Collection<CustbankVO> col =
				new BaseDAO().retrieveByClause(CustbankVO.class, where, new String[] { CustbankVO.PK_BANKACCSUB, CustbankVO.PK_CUST });
	...
	private static Map<String, String> queryBankaccPksByPsnPk(String... pk_psns) throws BusinessException {
		...
		Collection<PsnBankaccVO> col =
				new BaseDAO().retrieveByClause(PsnBankaccVO.class, where, new String[] { PsnBankaccVO.PK_BANKACCSUB, PsnBankaccVO.PK_PSNDOC });
		...
		if (!CmpUtils.isListNull(vos)) {
			for (PsnBankaccVO vo : vos) {
				map.put(vo.getPk_psndoc() + " " + vo.getPk_bankaccsub(), vo.getPk_psndoc() + " " + vo.getPk_bankaccsub());
			}
	...
	public static void dataIntegralityValidate(SettlementAggVO... settlementAggVOs) throws BusinessException {
		...
		for (SettlementAggVO settlementAggVO : settlementAggVOs) {
			...
			String billType = SettleUtils.getbilltype(pk_tradertype, pk_group);
			if ("F4".equals(billType) || "F5".equals(billType)) {
				if ("F5".equals(billType)) {

					ICmpPayBillQueryService cmpPayBillQueryService = NCLocator.getInstance().lookup(ICmpPayBillQueryService.class);
					...
					ICmpRecBillQueryService cmpRecBillQueryService = NCLocator.getInstance().lookup(ICmpRecBillQueryService.class);
	...
	private static void validateOPPInfo(SettlementAggVO settlementAggVO, BillAggVO billAggVO) throws BusinessException {
		...
		for (SettlementBodyVO settlementBodyVO : settlementBodyVOs) {
			...
			for (BillDetailVO billDetailVO : billdaDetailVOs) {
				...
				if (pk_billdetail != null && pk_billdetail.equals(billDetailVO.getPrimaryKey())) {

					String pk_oppAccount = settlementBodyVO.getPk_oppaccount();
					String pk_oppAccountBill = billDetailVO.getPk_account();
					if (!(pk_oppAccount == null && pk_oppAccountBill == null || pk_oppAccount != null && pk_oppAccount.equals(pk_oppAccountBill))) {
						throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("36070pubcmp_1", "3607pubcmp2-0013"));
					}
					...
					if (!(pk_trader == null && pk_Billtrader == null || pk_trader != null && pk_trader.equals(pk_Billtrader))) {
						...
						throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("36070pubcmp_1", "3607pubcmp2-0014"));
					}
	...
	private static void validateOPPInfo(SettlementAggVO settlementAggVO, RecBillAggVO recBillAggVO) throws BusinessException {
		SettlementBodyVO[] settlementBodyVOs = (SettlementBodyVO[]) settlementAggVO.getChildrenVO();
		...
		for (SettlementBodyVO settlementBodyVO : settlementBodyVOs) {
			...
			for (RecBillDetailVO billDetailVO : billdaDetailVOs) {
				...
				if (billDetailVO.getObjecttype() == null || Integer.valueOf(4).equals(billDetailVO.getObjecttype())) {
					continue;
				}
